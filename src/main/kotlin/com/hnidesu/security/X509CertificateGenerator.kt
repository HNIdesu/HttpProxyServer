...
class X509CertificateGenerator(private val mCertificate: X509Certificate, private val mPrivateKey: PrivateKey) {
    fun generateSslContext(hostname:String):SslContext {
        val subject=X500NameBuilder().addRDN(BCStyle.CN,hostname).build()
        val issuer=Certificate.getInstance(mCertificate.encoded).issuer
        ...
        val cert= CertificateFactory.getInstance("X509").generateCertificate(ByteArrayInputStream(
            X509v3CertificateBuilder(
                ...
                Date(),
                Date(System.currentTimeMillis()+24*3600*1000),
                subject,
        ...
        return SslContextBuilder.forServer(keyPair.private,cert,mCertificate).build()
    }
